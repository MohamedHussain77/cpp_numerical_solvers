...
class Solver_advection{
    ...
    public:
        Solver_advection(const Mesh& mesh_param,Field_u& u_param, Field_u& u_old_param, double a_param, double cfl_param, 
            double totaltime_param, NumericalMethod& method_param, BoundaryCondition& condition_param_l, BoundaryCondition& condition_param_r): mesh(mesh_param), a(a_param), cfl(cfl_param), delt(cfl*mesh.get_delx()/a),
            totaltime(totaltime_param), numsteps(totaltime/delt), u(u_param), u_old(u_old_param), method(method_param), condition_left(condition_param_l), condition_right(condition_param_r){}
